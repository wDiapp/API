...
const CarroForm = () => {
    ...
    useEffect(() => {
        const fetchMarcas = async () => {
            try {
                const response = await axios.get('http://localhost:8081/api/marcas'); // Substitua pela URL correta
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:8081/api/carros', carroData);
